...
def func_upload(root, f2):
    ...
    c_forwarding.pack()
    c_clock = Checkbutton(box,width=16,text = "Clock",font=("Comic Sans MS",9),variable=var_clock,onvalue=1,offvalue=0)
    c_clock.pack()
    c_registers = Checkbutton(box,width=16,text = "Registers",font=("Comic Sans MS",9),variable=var_register,onvalue=1,offvalue=0)
    c_registers.pack()
    ...
    #Making Button
    btn_run=Button(below, text="Run",anchor="center",font=("Comic Sans MS",12), padx=15,border=3, bg='#151515',fg="white", command=lambda: run_func(location,content))
